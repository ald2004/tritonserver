...
namespace {
    ...
    std::string backend_dir = "/opt/work/boe/tritonserver/backend/build/install";
...
#define no_argument 2
    ...
    TRITONSERVER_Error* StartHttpService(
...
#ifdef TRITON_ENABLE_LOGGING
    int ParseIntOption(const std::string arg)
    {
        return std::stoi(arg);
    }
    int ParseIntBoolOption(std::string arg)
    {
...
#endif  // TRITON_ENABLE_LOGGING
    bool ParseBoolOption(std::string arg)
    {
        std::transform(arg.begin(), arg.end(), arg.begin(), [](unsigned char c) {
            return std::tolower(c);
            });
...
#endif  // TRITON_ENABLE_TRACING
    int64_t ParseLongLongOption(const std::string arg)
    {
        return std::stoll(arg);
    }
    std::pair<int, uint64_t>ParsePairOption(const std::string arg)
    {
        int delim = arg.find(":");
    ...
    double ParseDoubleOption(const std::string arg)
    {
        return std::stod(arg);
    }
    std::tuple<std::string, std::string, std::string>ParseBackendConfigOption(const std::string arg)
    {
    ...
    bool StartTracing(nvidia::inferenceserver::TraceManager** trace_manager)
    {
    ...
    bool StopTracing(nvidia::inferenceserver::TraceManager** trace_manager)
    {
#ifdef TRITON_ENABLE_TRACING
